...
#1 host = os.environ.get("VTA_RPC_HOST", "192.168.2.99")
#2 port = int(os.environ.get("VTA_RPC_PORT", "9091"))
...
if env.TARGET == "te0802":
    ...
    #3 assert tvm.runtime.enabled("rpc")
    #4 remote = rpc.connect(host, port)
...
#vta.program_fpga(remote, bitstream="/home/eamicheal/tvmTE0802/vta/tutorials/bit/vta15.bit")
...
# We run the function and verify it against a numpy implementation to ensure correctness.
...
tvm.upload(temp.relpath("conv2d.o"))
f = tvm.load_module("conv2d.o")
...
# Initialize the data and kernel arrays randomly in the int range of (-128, 128] in NCHW layout
...
end1 = time.time()
print('execution time is: ',end1 - start1, 's')
